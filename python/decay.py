...
def beta_fn(m1, m2, unit=False, fallback=True):
    ...
    try:
        from basil_core.astro.orbit._decay import _beta_arr, _beta_sgl # ty: ignore[unresolved-import]
...
def peters_ecc_const(ecc,fallback=True):
    ...
    try:
        from basil_core.astro.orbit._decay import _peters_ecc_const_sgl # ty: ignore[unresolved-import]
        from basil_core.astro.orbit._decay import _peters_ecc_const_arr # ty: ignore[unresolved-import]
    except Exception as exc:
...
def peters_ecc_integrand(ecc, fallback=True):
    ...
    try:
        from basil_core.astro.orbit._decay import _peters_ecc_integrand_sgl # ty: ignore[unresolved-import]
        from basil_core.astro.orbit._decay import _peters_ecc_integrand_arr # ty: ignore[unresolved-import]
    except Exception as exc:
...
def orbital_period_of_m1_m2_a(m1, m2, a, unit=False, fallback=True):
    ...
    try:
        from basil_core.astro.orbit._kepler import _orbital_period_of_m1_m2_a_sgl # ty: ignore[unresolved-import]
        from basil_core.astro.orbit._kepler import _orbital_period_of_m1_m2_a_arr # ty: ignore[unresolved-import]
    except Exception as exc:
...
def merge_time_circ(m1, m2, a, unit=False, fallback=True):
    ...
    # Try to import C extension
    try:
        from basil_core.astro.orbit._decay import _merge_time_circ_sgl # ty: ignore[unresolved-import]
        from basil_core.astro.orbit._decay import _merge_time_circ_arr # ty: ignore[unresolved-import]
    except Exception as exc :
...
def a_of_m1_m2_forb(m1,m2,forb):
    ...
    if hasattr(forb, "unit"):
        if not forb.unit.is_equivalent(u.Hz):
            raise ValueError(
                f"forb should have units Hz, but has units {a.unit}") # ty: ignore[unresolved-reference]
    else:
        forb = forb * u.Hz
...
def eccentric_ODE_integrand(
    ...
    integrand : function
        The defined integrand function for our ODE solver
    """
    from basil_core.astro.orbit._decay import _orb_sep_evol_ecc_integrand_sgl # ty: ignore[unresolved-import]
    from basil_core.astro.orbit._decay import _orb_sep_evol_ecc_integrand_arr # ty: ignore[unresolved-import]
    # Check inputs
    if hasattr(m1,'unit'):
        m1 = m1.to('kg').value
    if hasattr(m2,'unit'):
...
def orbital_separation_evolve_circ_numpy(m1,m2,a0,evolve_time):
    ...
    if hasattr(m2, "unit"):
        ...
        m2 = m2 * u.kg
    if hasattr(a, "unit"): # ty: ignore[unresolved-reference]
        if not a.unit.is_equivalent(u.m): # ty: ignore[unresolved-reference]
            raise ValueError(
                f"a should have units m, but has units {a.unit}") # ty: ignore[unresolved-reference]
    else:
        a = a * u.m # ty: ignore[unresolved-reference]
    return np.power(np.power(a0,4) - 4 * beta_fn(m1,m2) * t,1/4) # ty: ignore[unresolved-reference]
...
def orbital_separation_evolve_circ(m1,m2,a0,evolve_time,fallback=True,unit=False):
    ...
    # Try to import C extensions
    try:
        from basil_core.astro.orbit._decay import _orb_sep_evol_circ_sgl # ty: ignore[unresolved-import]
        from basil_core.astro.orbit._decay import _orb_sep_evol_circ_arr # ty: ignore[unresolved-import]
    except Exception as exc:
...
def orbital_separation_evolve_sgl(
    ...
    method : str
        Method for evaluating merge times
        BDF is the fastest one in my tests (not going all the way to merger)
    return_ecc : bool
    ...
    if method in IVP_METHODS:
        # Solve ODE way
        ivp_out, err = eccentric_ODE_integration(m1,m2,a0,e0,Teval=evolve_time)
        ef = ivp_out.y[-1]
        if np.size(ef) > 1:
            ef = ef[-1]
